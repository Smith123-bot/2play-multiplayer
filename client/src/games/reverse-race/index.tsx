...
import { REVERSE_RACE_METADATA, type GameAction } from '@2play/shared';
...
export interface ReverseRacePublicState {
  ...
  rows: number;
  grid: ReverseCell[][];
  objective: { kind: string; label: string; targetMs?: number; targetCoins?: number };
  finishOrder: string[];
  endsAt: number | null;
  ...
  players: Record<
    string,
    { x: number; y: number; score: number; roundScore: number; coins: number; finished: boolean; disconnected: boolean }
  >;
}
...
const CELL: Record<ReverseCell, string> = {
  wall: 'bg-slate-700/90',
  floor: 'bg-slate-200/10',
  finish: 'bg-emerald-400/50',
  coin: 'bg-amber-300/70',
  zone: 'bg-orange-400/40',
};

function ReverseRaceGame({
  state,
  players,
  myPlayerId,
  sendAction,
  play,
  vibrate,
}: GameComponentProps<ReverseRacePublicState>) {
  ...
  useEffect(() => {
    ...
    if (!lastEvent) return;
    if (lastEvent.startsWith('coin:') && lastEvent.includes(myPlayerId ?? '')) {
      play('score');
      vibrate('success');
    } else if (lastEvent.startsWith('finish:') && lastEvent.includes(myPlayerId ?? '')) {
      play('victory');
      vibrate('victory');
    } else if (lastEvent.startsWith('objective:') || lastEvent === 'round-end') {
      play('notification');
    }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="card grid place-items-center p-10 text-center">
          <p className="animate-pulse text-sm text-slate-400">Flipping the next objective…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Badge tone="accent">{state.objective.kind}</Badge>
        <Badge tone="warning">Tokens {me?.coins ?? 0}</Badge>
        {me?.finished ? <Badge tone="success">Finished</Badge> : null}
      </div>
      <div className="card p-3 text-center">
        <p className="text-xs uppercase tracking-wider text-slate-400">This round</p>
        <p className="mt-1 text-sm font-semibold text-white">{state.objective.label}</p>
      </div>
      <div
        role="grid"
        aria-label="Reverse race track"
        className="touch-none mx-auto grid w-full max-w-lg gap-px overflow-hidden rounded-2xl border border-white/10 bg-black/50 p-1 select-none"
      ...
      >
        {state.grid.flatMap((row, y) =>
          row.map((cell, x) => {
            ...
            return (
              <div key={`${x}:${y}`} className={cn('relative aspect-square', CELL[cell])}>
      ...
      <p className="text-center text-xs text-slate-500">
        First is not always best. The server owns the objective, the tokens and the finish times.
      </p>
    </div>
  );
}

export const reverseRaceClient: ClientGameModule = {
  metadata: REVERSE_RACE_METADATA,
  Component: ReverseRaceGame as unknown as ComponentType<GameComponentProps<never>>,
};
